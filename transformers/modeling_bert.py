...
class BertAttention(nn.Module):
    ...
    def graft_heads(self, idx):
        # n, a = self.self.num_attention_heads, self.self.attention_head_size
        n, a = 12, 64
        index = torch.arange(n*a).reshape(n, a)[idx].view(-1).contiguous().long()

        def graft_head_matrix(linearLayer, index, dim=0):
            index = index.to(linearLayer.weight.device)
            # index_1 =  torch.LongTensor([1, 2, 0] * 4).to(linearLayer.weight.device)
            # index_2 =  torch.LongTensor([2, 0, 1] * 4).to(linearLayer.weight.device)
            # W = 0.1 * (linearLayer.weight.index_select(dim, index).clone().detach() + linearLayer.weight.index_select(dim, index_1).clone().detach() 
            #     + linearLayer.weight.index_select(dim, index_2).clone().detach())/3 + 0.9 * linearLayer.weight.clone().detach()
            W = 0.1 * linearLayer.weight.index_select(dim, index).clone().detach() + 0.9 * linearLayer.weight.clone().detach()
            if linearLayer.bias is not None:
                if dim == 1:
                    b = linearLayer.bias.clone().detach()
                else:
                    # b = 0.1 * (linearLayer.bias[index].clone().detach() + linearLayer.bias[index_1].clone().detach() + 
                    #     linearLayer.bias[index_2].clone().detach())/3 + 0.9 * linearLayer.bias.clone().detach()
                    b = 0.1 * linearLayer.bias[index].clone().detach() + 0.9 * linearLayer.bias.clone().detach()
        ...
        graft_head_matrix(self.self.query, index)
        graft_head_matrix(self.self.key, index)
        graft_head_matrix(self.self.value, index)
        graft_head_matrix(self.output.dense, index, dim=1)
...
class BertIntermediate(nn.Module):
    ...
    def graft_neurons(self, index, dim=0):
        index = index.to(self.dense.weight.device)
        W = 0.1 * self.dense.weight.index_select(dim, index).clone().detach() + 0.9 * self.dense.weight.clone().detach()
        if self.dense.bias is not None:
            if dim == 1:
                b = self.dense.bias.clone().detach()
            else:
                b = 0.1 * self.dense.bias[index].clone().detach() + 0.9 * self.dense.bias.clone().detach()
        self.dense.weight.requires_grad = False
        self.dense.weight.copy_(W.contiguous())
        self.dense.weight.requires_grad = True
        if self.dense.bias is not None:
...
class BertOutput(nn.Module):
    ...
    def graft_neurons(self, index, dim=1):
        index = index.to(self.dense.weight.device)
        W = 0.1 * self.dense.weight.index_select(dim, index).clone().detach() + 0.9 * self.dense.weight.clone().detach()
        if self.dense.bias is not None:
            if dim == 1:
